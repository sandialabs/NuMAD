function [output,outputAllFiles]=runIECDLC6p2(params,output)
global fastPath
global adamsPath
CaseName='IECDLC6p2EWM50';
...
if 0 % run full set
    windLabels = {};
    for yawAngle=-180:5:175
        if yawAngle >= 0
            windLabels{end+1}=['EWM50+' num2str(yawAngle) '.wnd']
        else
            windLabels{end+1}=['EWM50' num2str(yawAngle) '.wnd']
        end
    end
else % subset, for efficiency
    windLabels={'EWM50+100.wnd' 'EWM50+95.wnd' 'EWM50+90.wnd' 'EWM50+85.wnd' 'EWM50+80.wnd'...
        'EWM50-100.wnd' 'EWM50-95.wnd' 'EWM50-90.wnd' 'EWM50-85.wnd' 'EWM50-80.wnd'}
    warning('runIECDLC6p2 is running only a single case in this configuration.  Please verify that it is the correct case.')
end
