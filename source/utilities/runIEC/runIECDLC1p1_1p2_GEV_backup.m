function [output,outputAllFiles1p2,Dminer]=runIECDLC1p1_1p2_GEV_backup(params,matData,output,DLCoptions)
global fastPath
...
if any(contains(DLCoptions,'1.1')) || any(contains(DLCoptions,'all'))
    ...
    % Define the  output channels for which the 50-year extrapolated value are desired
    labels = {'RootMxb1' 'RootMyb1' 'OoPDefl1'};
%     labels = fst.OutList;
    
    [output,outputAllFiles1p2] = IECDLC1p1_extrapolation(ctr,labels,CaseName,params,weibullParams,cs,EIs,EA_normal,output);
end

% perform the design load case 1.2 fatigue life calculation
Dminer = 999;
if any(contains(DLCoptions,'1.2')) || any(contains(DLCoptions,'all'))
    [Dminer,output,outputAllFiles1p2] = IECDLC1p2_fatigue(CaseName,params,ctr,labels,matData,avgws,runcrunch,saverccdata,fst,cs,EIs,EA_normal,output);
end
...
function [output,outputAllFiles] = IECDLC1p1_extrapolation(ctr,labels,CaseName,params,weibullParams,cs,EIs,EA_normal,output)
% =========================================================================
...
for ii = 1:length(names)
    ...
    for cc = 1:nChannels
        ...
        for jj = 1:length(peaks{ii})
            ...
            % mean and standard deviation of peak set
            peakMax_mean = nanmean(peaksMax);
            peakMax_stdev = nanstd(peaksMax);
        end
        keyboard
        % calculate the 50 year value for the data channel in labels
        tempMax = get50YearLoad(maxPeakFit{ii},weibullParams,params,peakMax_mean,peakMax_stdev,timeRatio);
...
function F_50year = get50YearLoad(peakFit,weibullParams,params,peak_mean,peak_stdev,timeRatio)
    % 
    X1 = peak_mean;
    X2 = peak_mean + peak_stdev;
    ...
    if(it == 50)
        warning('Couldnt find suitable Y2')
    end
    Fmid = 0.5*(X1 + X2);
    Ymid = probEx(Fmid,peakFit,weibullParams,params) - timeRatio;
    it = 0;
...
function [binMaxPeak] = findPeaks1p1(data,labels,isMinimum,plotTimeSeries,time)
...
if plotTimeSeries
    ...
    try % plot peaks if they exist
        plot(time(index1),gains.*dataMax(index1),'x')
...
function [Dminer,output,outputAllFiles] = IECDLC1p2_fatigue(CaseName,params,ctr,labels,matData,avgws,runcrunch,saverccdata,fst,cs,EIs,EA_normal,output)
% =========================================================================
...
Dminer=MFatigue_ble(cs,EIs,matData,simtime,avgws,params.designLife,params.sf_fat);
