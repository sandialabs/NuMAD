function structOpt_stiff(blade,lb,ub,A,b,pop,gen,restartFile,useParallel)
...
if 0 % call the genetic algorithm
...
else % Do manual analysis of best iteration to save the final optimization
    x=[459.706451 0.056976 15.414284 0.156272 9.324243 18.057269 13.474714 4.970696]
    for i=1:size(x,1)
...
designcriteria_OoPDefl=1.328;
designcriteria_Mass = 650 - 10;   % (minus mass used for instrumentation) kg
designcriteria_FFreq = 48/60 * 3 * 1.1;  % for 13m rotor
designcriteria_EFreq = 48/60 * 3 * 1.1;  % for 13m rotor
...
% Maximum stiffness design by minimizing tip deflection, ignoring fatigue, ignoring buckling, 
f= designvarOoPDefl * penaltyMass * penaltyFFreq * penaltyEFreq;
params=[f,designvarOoPDefl,designvarFFreq,designvarMass];

% write results to layupDesignProgress.txt
cd(hm)
fid=fopen('layupDesignProgress.txt','a');
fprintf(fid,'%s, (value/penalty) %2.2f; OoPDefl [m] %2.3f; Flap Freq [hz] %2.3f; Actual mass [kg] %6.0f\n',datestr(now),params);
fclose(fid);
