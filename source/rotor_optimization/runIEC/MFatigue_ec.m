function out = MFatigue_ble(wt,rccdata,cs,EIs,matData,params)
...
%         if ch ==1 || ch==11 || ch==31 || ch==32
            E=2.87e+10;
            uts=4.36e+08*gamma_ms;
            ucs=-3.43e+08*gamma_ms;
%             uts=4.36e+08;
%             ucs=-3.43e+08;
            eps_uts = uts/E;
            eps_ucs = ucs/E;
%         end
        ...
        for w=1:size(rccdata,2)
            ...
            if contains(data.label,'M') && contains(data.label,'r0')
                % EI and neutral axis location, c, for edgewise moment (Mxb in FAST)
                cy = cs(chSpan,1);
                EIedge = EIs(chSpan,1);
                c=cy; EI=EIedge;
            elseif contains(data.label,'M') && contains(data.label,'r90')
...
%                                 sigma = sigma_a/(1-sigma_m/sigma_u);
                    ...
                    case 'Shifted Goodman'
                        ...
                        Nf=((uts+abs(ucs)-abs(2*sigma_m*gamma_ms*FSloads-uts+abs(ucs)))/(2*sigma_a*gamma_mf*FSloads))^m;
                    case 'Fatigue Data'
...
% %             end
        ...
        if  ch>30 
            if mod(ch,2) %if jj is odd
                direction =int2str(0);
                cy = cs(chSpan,1);
                EIedge = EIs(chSpan,1);
                c=cy; EI=EIedge;
            else 
                direction =int2str(90);  
                cx = cs(chSpan,2);
                EIflap = EIs(chSpan,2);
                c=cx;EI=EIflap;
            end
            markov=getMomentMarkov(rccdata,wt,Yr,simtime,16,chSpan,direction);
            
            MthetaFactor=c/EI * E;
            
            LthetaWithFactor=markov; %Initialize for every layer
            ...
            test=LthetaWithFactor;
            save('test','test')
            out(ch,mat)=shiftedGoodman(LthetaWithFactor,uts,ucs,m,gamma_ms,gamma_mf);  
...
%             load('LthetaPlus90')
%             MthetaFactor=c/EI * E;
%             LthetaWithFactor=LthetaPlus90; %Initialize for every layer
%             LthetaWithFactor(1,:)=LthetaWithFactor(1,:)*MthetaFactor; %Means
%             LthetaWithFactor(:,1)=abs(LthetaWithFactor(:,1)*MthetaFactor); %Amplitudes
%             out(ch,mat)=shiftedGoodman(LthetaWithFactor,uts,ucs,m,gamma_ms,gamma_mf);
%         else
