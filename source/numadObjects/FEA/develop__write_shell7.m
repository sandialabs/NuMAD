...
TotalShearwebs = numel(blade.shearweb);
...
% %         grid on; axis equal
        ...
        data.station(kStation).coords = af_out; % will this affect other uses? Does app get saved back to NuMAD?? Is this necessary??
        nCoordPairs = size(station.coords,1);
% %     end
    %ble >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
for kStation = 1:numel(SkinAreas)
    stationIB = data.station(kStation);
    stationOB = data.station(kStation+1);
...
for k = 1:TotalShearwebs
    compsInModel = [compsInModel; {blade.shearweb(k).Material}];  %#ok
end
...
%     fprintf(fid,'\ncsys,0');
    ...
    for kStation = 1:TotalStations
        isFlatback = strcmp(data.station(kStation).TEtype,'flat');
    ...
    for kStation = 1:TotalStations-1
        ...
        thisIsFlatback = strcmp(data.station(kStation).TEtype,'flat');
        nextIsFlatback = strcmp(data.station(kStation+1).TEtype,'flat');
    ...
    for kStation = 1:TotalStations-1
        for kArea = 1:numel(SkinAreas(kStation).startIB)
            ...
            % function cartesian_check(app,startIB,endIB,endOB,startOB)
                ...
                tolerance = 0.005 * data.station(1).Chord;
                ...
                station = data.station(kStation+1);
...
%                     mat = app.matdb(n);
    ...
    for kShearweb = 1:TotalShearwebs
      ...
      BeginStation = blade.shearweb(kShearweb).BeginStation;
      EndStation = blade.shearweb(kShearweb).EndStation;
      firstKP = (BeginStation-1)*1000 + KeyPoints(BeginStation).DP(1+blade.shearweb(kShearweb).Corner(1));
      secondKP = (BeginStation-1)*1000 + KeyPoints(BeginStation).DP(1+blade.shearweb(kShearweb).Corner(2));
      thirdKP = (EndStation-1)*1000 + KeyPoints(EndStation).DP(1+blade.shearweb(kShearweb).Corner(3));
      fourthKP = (EndStation-1)*1000 + KeyPoints(EndStation).DP(1+blade.shearweb(kShearweb).Corner(4));
