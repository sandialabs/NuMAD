function [output,outputAllFiles]=runIECDLC1p4_par_work(params,output)

CaseName='IECDLC1p4ECD';

% Switches:
fastsim='fast simulink';
fastsim='fast';
% create the wind file labels
if true % run full set
    windLabels={'ECD+R-2.0.wnd',...
        'ECD+R.wnd',...
        'ECD+R+2.0.wnd',...
        'ECD-R-2.0.wnd',...
        'ECD-R.wnd',...
    ...
    windLabels={'ECD-R.wnd'};
    warning('runIECDLC1p4 is running only a single case in this configuration.  Please verify that it is the correct case.')
end
...
% CK parfor or for
for ii=1:length(ctr.windLabels)
%     % ble <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
%     % ble >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    if params.simulate
        fst=readFastMain([params.fstfn id '.fst']);
        ad=readFastAD([fst.ADFile(2:end-8) id '_AD.ipt']);
        % prepare and perform aeroelastic simulation
        ad.WindFile=['"' 'wind\' ctr.windLabels{ii} '"'];
        fst.SimCtrl.TMax=params.delay+100; %ble: fix for numerical errors around 90-sec.  fst.SimCtrl.TMax=100;
        fst.ADFile=['"' CaseName '_' fst.ADFile(2:end-8) id '_AD.ipt' '"'];% ble: change the AD filename
%         if ~strcmp(fastsim,'fast simulink')
...
%     output2{cc} = output;
    output2{cc}.MaxFlapBendingMoment=[]; % set up output structure
    output2{cc}.MaxRotorFlapBendingMoment=[];
end
%CK no parfor
% determine the maximum/minimum loads for each output file
for cc = 1:length(outName)
    output2{cc} = sortExtremes(params,output2{cc},CaseName,outName(cc),cs,EIs);
end
