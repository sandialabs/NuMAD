function structOpt_mass(bladeFile,lb,ub,A,b,pop,gen,restartFile,useParallel)
...
if 1 % call the genetic algorithm
    [x,fval,exitflag,output,population,scores] = ga(@(x)designBladeFcn(x,blade,useParallel),...
    ...
    for i=1:size(x,1)
        f(i,1) = designBladeFcn(x(i,:), blade, false);
        save tmp x f
    end
...
%%
function f = designBladeFcn(x, blade, useParallel)
% <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
