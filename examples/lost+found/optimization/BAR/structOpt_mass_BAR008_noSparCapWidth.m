function structOpt_mass_BAR008_noSparCapWidth(blade,lb,ub,compi,imatch,A,b,pop,gen,restartFile,useParallel)
...
%             options = optimoptions('patternsearch','UseParallel', true, 'UseCompletePoll', true, 'UseVectorized', false,'MaxFunctionEvaluations',1000);
        ...
        case 'particle swarm'
            ...
            disp(' ')
            [x,fval,exitflag,output] = particleswarm(@(x)designBladeFcn(x,compi,imatch,blade,fast_gage,useParallel,false),nVar,lb,ub,options);
...
%%
function f = designBladeFcn(x,compi,imatch,blade,fast_gage,useParallel,saveBlade)
% <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
for i=compi
    if i~=imatch(ctMatch,2)
        nvar = length(blade.components(i).cp); %Number of design varibles in blade.components(i)
        for j = 1:nvar
            if j< nvar
                ct = ct+1; %Do not increment the design variable count because the last cp will be set the second to last point due to updateBOM bug.
            end
            blade.components(i).cp(j,2)=x(ct);
        end
    else
        blade.components(i).cp(:,2)=blade.components(imatch(ctMatch,1)).cp(:,2);
        ctMatch=ctMatch+1;
    end
end
...
%     failIndex=zeros(1,length(loads_table));
    ...
    for ii = 1:length(loads_table)
        ...
        [feaOutput]=layupDesign_ANSYSbuckling_ble(blade,loads_table{ii},config,ii);
...
function plotBladeDesignVar(blade,sym)
ctMatch=1;
