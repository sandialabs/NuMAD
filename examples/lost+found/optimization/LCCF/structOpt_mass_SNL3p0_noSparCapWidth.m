function structOpt_mass_SNL3p0_noSparCapWidth(blade,lb,ub,A,b,pop,gen,restartFile,useParallel)
...
if 1 % ================= Perform Buckling Analysis =================
    ...
    config.ansys.nonlinear = 1;
    config.ansys.wrinkling = 1;
    ...
    loads_table = FastLoads4ansys(output,fast_gage);
    ...
    for ii = 1:length(loads_table)
        ...
        [feaOutput,~]=layupDesign_ANSYSbuckling_ble(blade,loads_table{ii},config);
    ...
    designVar.Buckle = minimum(buckling_loadFactor);
    ...
    designVar.Wrinkle = minimum(wrinkling_loadFactor);
